...
public class EmployeDao {
    ...
    public Employe getEmployeDispo(Agence a, int heure) {
        ...
        TypedQuery<Employe> query = em.createQuery(
                "SELECT e FROM Employe e WHERE e.agence =:a AND e.disponible = TRUE AND e.heureDeb <= :heure AND e.heureFin > :heure",
                Employe.class);
    ...
    public Employe seConnecter(String mail, String mdp) {
        ...
        TypedQuery<Employe> query = em
                .createQuery("SELECT e FROM Employe e WHERE e.mail=:mail AND e.motDePasse= :motPasse", Employe.class);
